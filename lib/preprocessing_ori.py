...
def graphs_dataset(dataset, HyperParams, tr_snap_range = None, print_info=False, truncate_mesh=False):
    """
    ...
    if truncate_mesh:
        ...
        for idx, ele in enumerate(edges_0):
            if ele in range(29, 57):
                ele -= 29
            else:
                ele -= 500 - (57-29)
            edges_0[idx] = ele
        for idx, ele in enumerate(edges_1):
            if ele in range(29, 57):
                ele -= 29
            else:
                ele -= 500 - (57-29)
...
def get_train_dataset_from_list(data_list, idx_list, HyperParams, truncate_mesh=False):
    Net_graph_loader = []
...
def get_test_dataset_from_list(test_data_list, idx_list, HyperParams, truncate_mesh=False):
    test_graph_loaders = []
    test_scaler_alls = []
    test_xyzs = []
    test_VAR_alls = []
